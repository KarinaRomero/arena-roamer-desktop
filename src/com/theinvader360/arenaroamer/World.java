...
public class World {
	...
	public void update(float delta, int dir) {
		handleInput(delta, dir);
	}
	...
	private void handleInput(float delta, int dir) {
		// Android controls
...
//		}
		...
		// Desktop controls
		if (Gdx.app.getType() == ApplicationType.Desktop) {
			if(Input.Keys.UP==dir) player.moveForward(delta);
			if(Input.Keys.DOWN==dir) player.moveBackward(delta);
			if(Input.Keys.LEFT==dir) player.turnLeft(delta);
			if(Input.Keys.RIGHT==dir) player.turnRight(delta);
			if(Input.Keys.Z==dir) player.strafeLeft(delta);
			if(Input.Keys.X==dir) player.strafeRight(delta);
			if(Gdx.input.isKeyPressed(Input.Keys.R)) restartLevel();
			if(Gdx.input.isKeyPressed(Input.Keys.Q)) restartGame();
		}
	}
